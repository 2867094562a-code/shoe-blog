...
function postDataToObject() {
  ...
  data.is_pinned = postForm?.elements?.is_pinned?.checked ? 'true' : 'false';
  data.is_featured = postForm?.elements?.is_featured?.checked ? 'true' : 'false';
  return data;
}
...
function renderPostTable() { $('#postTable').innerHTML = posts.length ? posts.map(p => `<div class="admin-post-row reveal-up in-view"><div><h3>${escapeHtml(p.title)} ${p.is_pinned ? '<small class="status-pill">置顶</small>' : ''} ${p.is_featured ? '<small class="status-pill">精选</small>' : ''}</h3><p class="muted">${escapeHtml(p.status)} · ${fmtDate(p.created_at)} · ${escapeHtml(p.category || '未分类')} · ❤ ${Number(p.likes || 0)} · ${cleanPath(p.slug)}</p></div><div class="row-actions"><a class="small-btn" href="${cleanPath(p.slug)}" target="_blank">查看</a><button data-edit="${p.id}">编辑</button><button class="danger" data-delete="${p.id}">删除</button></div></div>`).join('') : '<p class="muted">暂无文章。</p>'; $$('[data-edit]').forEach(btn => btn.addEventListener('click', () => editPost(btn.dataset.edit))); $$('[data-delete]').forEach(btn => btn.addEventListener('click', () => removePost(btn.dataset.delete))); }
async function editPost(id) { const { post } = await api(`/api/admin/posts/${id}`); $('#editorTitle').textContent = `编辑文章：${post.title}`; postForm.id.value = post.id; postForm.title.value = post.title || ''; postForm.slug.value = post.slug || ''; postForm.category.value = post.category || ''; postForm.tags.value = Array.isArray(post.tags) ? post.tags.join(',') : (post.tags || ''); updateArticleTaxonomyUI(); postForm.cover.value = post.cover || ''; postForm.excerpt.value = post.excerpt || ''; postForm.is_pinned.checked = Boolean(post.is_pinned); postForm.is_featured.checked = Boolean(post.is_featured); postForm.seo_title.value = post.seo_title || ''; postForm.seo_description.value = post.seo_description || ''; postForm.seo_image.value = post.seo_image || ''; postForm.seo_noindex.checked = Boolean(post.seo_noindex); postForm.content.value = post.content || ''; setBlocksFromMarkdown('post', post.content || ''); postForm.status.value = post.status || 'published'; renderPostPreview(); setAutosaveStatus(localStorage.getItem(autosaveKey()) ? '有自动草稿可恢复' : '左写右看'); switchTab('posts'); postForm.scrollIntoView({ behavior: 'smooth', block: 'start' }); }
