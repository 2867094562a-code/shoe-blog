...
function updateArticleTaxonomyUI() { if (!postForm) return; const cat = normalizeCategoryInput(postForm.category.value); if (postForm.category.value !== cat) postForm.category.value = cat; const catBox = $('#postCategoryChip'); if (catBox) catBox.innerHTML = cat ? `<span class="taxonomy-token">@${escapeHtml(cat)} <button type="button" data-clear-post-category>×</button></span>` : '未选择分类'; const tags = currentPostTags(); const tagBox = $('#postTagChips'); if (tagBox) tagBox.innerHTML = tags.length ? tags.map(t => `<span class="taxonomy-token">${escapeHtml(tagLabel(t))} <button type="button" data-remove-post-tag="${escapeHtml(t)}">×</button></span>`).join('') : '未添加标签'; $('#postCategoryChip')?.querySelector('[data-clear-post-category]')?.addEventListener('click', () => { postForm.category.value = ''; updateArticleTaxonomyUI(); renderPostPreview(); }); $$('[data-remove-post-tag]', tagBox || document).forEach(btn => btn.addEventListener('click', () => { setCurrentPostTags(currentPostTags().filter(t => t !== btn.dataset.removePostTag)); updateArticleTaxonomyUI(); renderPostPreview(); })); }
function renderTaxonomyEditors() { syncTaxonomyInputs(); const catWrap = $('#categoryEditorList'); if (catWrap) catWrap.innerHTML = taxonomyState.categories.length ? taxonomyState.categories.map((name, index) => `<div class="taxonomy-editor-row card-lite"><input data-tax-edit="category" data-tax-index="${index}" value="${escapeHtml(name)}"><div class="row-actions"><button type="button" data-tax-up="category" data-tax-index="${index}" ${index === 0 ? 'disabled' : ''}>上移</button><button type="button" data-tax-down="category" data-tax-index="${index}" ${index === taxonomyState.categories.length - 1 ? 'disabled' : ''}>下移</button><button type="button" class="danger" data-tax-delete="category" data-tax-index="${index}">删除</button></div></div>`).join('') : '<p class="muted">暂无分类。</p>'; const tagWrap = $('#tagEditorList'); if (tagWrap) tagWrap.innerHTML = taxonomyState.tags.length ? taxonomyState.tags.map((name, index) => `<div class="taxonomy-editor-row card-lite"><input data-tax-edit="tag" data-tax-index="${index}" value="${escapeHtml(name)}"><div class="row-actions"><button type="button" data-tax-up="tag" data-tax-index="${index}" ${index === 0 ? 'disabled' : ''}>上移</button><button type="button" data-tax-down="tag" data-tax-index="${index}" ${index === taxonomyState.tags.length - 1 ? 'disabled' : ''}>下移</button><button type="button" class="danger" data-tax-delete="tag" data-tax-index="${index}">删除</button></div></div>`).join('') : '<p class="muted">暂无标签。</p>'; $$('[data-tax-edit]').forEach(input => input.addEventListener('input', () => { const key = input.dataset.taxEdit === 'category' ? 'categories' : 'tags'; const norm = input.dataset.taxEdit === 'category' ? normalizeCategoryInput(input.value) : normalizeTagInput(input.value); taxonomyState[key][Number(input.dataset.taxIndex)] = norm; taxonomyState[key] = uniqueItems(taxonomyState[key].filter(Boolean)); syncTaxonomyInputs(); })); $$('[data-tax-delete]').forEach(btn => btn.addEventListener('click', () => { const key = btn.dataset.taxDelete === 'category' ? 'categories' : 'tags'; taxonomyState[key].splice(Number(btn.dataset.taxIndex), 1); renderTaxonomyEditors(); })); $$('[data-tax-up]').forEach(btn => btn.addEventListener('click', () => { const key = btn.dataset.taxUp === 'category' ? 'categories' : 'tags'; const i = Number(btn.dataset.taxIndex); if (i > 0) [taxonomyState[key][i - 1], taxonomyState[key][i]] = [taxonomyState[key][i], taxonomyState[key][i - 1]]; renderTaxonomyEditors(); })); $$('[data-tax-down]').forEach(btn => btn.addEventListener('click', () => { const key = btn.dataset.taxDown === 'category' ? 'categories' : 'tags'; const i = Number(btn.dataset.taxIndex); if (i < taxonomyState[key].length - 1) [taxonomyState[key][i + 1], taxonomyState[key][i]] = [taxonomyState[key][i], taxonomyState[key][i + 1]]; renderTaxonomyEditors(); })); }
function addCategoryFromInput(value) { const cat = normalizeCategoryInput(value); if (!cat) return; if (!taxonomyState.categories.includes(cat)) taxonomyState.categories.push(cat); renderTaxonomyEditors(); showIsland(`已新增分类 @${cat}`); }
function addTagFromInput(value) { const tag = normalizeTagInput(value); if (!tag) return; if (!taxonomyState.tags.includes(tag)) taxonomyState.tags.push(tag); renderTaxonomyEditors(); showIsland(`已新增标签 ${tag}`); }
